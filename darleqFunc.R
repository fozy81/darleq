darleqFunc <- function(diatomTDI2){
  ...
  diatomTDI2$LTDI1[which(is.na(diatomTDI2$LTDI1))] <- 0
  diatomTDI2$LTDI2[which(is.na(diatomTDI2$LTDI2))] <- 0
  ...
  diatomTDI2$LTDI2[diatomTDI2$LTDI2 == "NA"] <- 0
  diatomTDI2$LTDI1[diatomTDI2$LTDI1 == "NA"] <- 0
 ...
 dataf <- lapply(split(diatomTDI2, diatomTDI2$SampleID), function(TDI){ #creates a new function which applies a series of commands on the split data by sample ID  
  ...
  tdi4SumAbund <- sum(TDI$Abundance[TDI$TDI4 > 0])# sum abundance of scoring TDI4 taxa
  allTaxaAbund <- nrow(TDI) # count all taxa (the number of rows i.e. should be a row per taxa)
  allTaxaAbund3 <- sum(TDI$Abundance[TDI$TDI3 >= 0]) # count all taxa abundance checking TDI.x field is not null/na 
  ...
  motileAbund <- sum(TDI$Abundance[TDI$Motile == TRUE]) # sum abundance of motile taxa
  ...
  motileAbund3 <- sum(TDI$Abundance[TDI$Motile == TRUE]) # sum abundance of motile taxa
  ...
  LTDI3SumAbund <- sum(TDI$scoreL3) # issue with duplicate taxa in NEMS
