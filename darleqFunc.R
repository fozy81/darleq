...
darleqFunc <- function(diatomTDI2){
  ...
  diatomTDI2$eLTDI2 <- ifelse(diatomTDI2$Alk >= 10 & diatomTDI2$Alk <= 50,35, diatomTDI2$Alk)
  diatomTDI2$eLTDI2[diatomTDI2$Alk < 10] = 22 ## Create LTDI2 reference/expected diatom index value based on three Alkalinity bands
  diatomTDI2$eLTDI2[diatomTDI2$Alk > 50] = 42
  ...
  dataf <- lapply(split(diatomTDI2, diatomTDI2$SampleID), function(TDI){ #creates a new function which applies a series of commands on the split data by sample ID  
    ...
    TDI4 <- ((as / tdi4SumAbund)*25) - 25 
    TDI3 <- ((as3 / tdi3SumAbund)*25) - 25
    ...
    EQR <- (100-TDI4)/(100-eTDI)
    tdi3EQR <-  (100-TDI3)/(100-eTDI3)
    SiteID <- unique(TDI$SiteID)
    date <- unique(TDI$Date)
    sampleID <- unique(TDI$SampleID)
    #loc <- unique(TDI$Loc.x)
    
    ### Loch LTDI2 scores:
    
    sumLTDI2 <- sum(TDI$Abundance[TDI$LTDI2 > 0],na.rm=TRUE) #  should this exclude zero scoring taxa? -
    LTDI4SumAbund <- sum(TDI$scoreL4,na.rm=TRUE)
    w <- LTDI4SumAbund / sumLTDI2
    LTDI2 <- (w * 25) - 25
    eqrLTDI2 <- (100 - (LTDI2)) / (100 - (unique(TDI$eLTDI2)))
    eLTDI2 <- unique(TDI$eLTDI2)
    
    ### Loch LTDI1 scores:
    
    totalabundsum <- sum(TDI$Abundance,na.rm=TRUE)
    sumLTDI1 <- sum(TDI$Abundance[TDI$LTDI1 > 0],na.rm=TRUE) # should this exclude zero scoring taxa? -
    LTDI3SumAbund <- sum(TDI$scoreL3,na.rm=TRUE) # issue with duplicate taxa in NEMS
    w2 <- LTDI3SumAbund / sumLTDI1
    lochTDI3 <- (w2 * 25) - 25
    eqrLTDI1 <- (100 - (lochTDI3)) / (100 - (unique(TDI$LochRefValue)))
    ...
    outDF <- data.frame(sumAbund =sumAbund,
                        ...
                        percentagePlanktic=plankticPercent,
